...
public class Block
{
  ...
  public string BlockHashString => Convert.ToHexString(BlockHash ?? new byte[0]);
  public string PreviousBlockHashString
  {
    get
    {
      if (PreviousBlockHash == null) return "None";
      return Convert.ToHexString(PreviousBlockHash ?? new byte[0]);
    }
  }
  public string MessageSub => Message.Length > 50 ? Message.Substring(0, 50) : Message;
